...
class BallReactorGradedBlanket(paramak.Reactor):
    ...
    def __init__(
        ...
        self.blanket_rear_wall_radial_thickness = \
            blanket_rear_wall_radial_thickness
        self.number_of_blanket_layers = number_of_blanket_layers
    ...
    def _make_blankets_layers(self):
        ...
        blanket_layer_thickness = self.blanket_radial_thickness / self.number_of_blanket_layers

        for i in range(self.number_of_blanket_layers):

            blanket_layer = paramak.BlanketFP(
                plasma=self._plasma,
                thickness=blanket_layer_thickness,
                offset_from_plasma=[e + self.firstwall_radial_thickness + i*blanket_layer_thickness for e in self.offset_from_plasma],
                start_angle=-180,
                stop_angle=180,
                rotation_angle=self.rotation_angle,
                material_tag="blanket_layer_"+str(i+1)+"_mat",
                stp_filename="blanket_layer_"+str(i+1)+".stp",
                stl_filename="blanket_layer_"+str(i+1)+".stl",
                cut=[self._center_column_cutter],
            )
            self._blanket_layers.append(blanket_layer)
        ...
        components = []
        components.append(self._firstwall)
        components.append(self._blanket_rear_wall)
        for i in self._blanket_layers:
            components.append(i)
    ...
    def _make_divertor(self):
        ...
        for component in [
                self._firstwall,
                self._blanket_envelope,
                self._blanket_rear_wall]:
            component.cut.append(self._divertor)

        for blanket_layer in self._blanket_layers:
            blanket_layer.cut.append(self._divertor)
