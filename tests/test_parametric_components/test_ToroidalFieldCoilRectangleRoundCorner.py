...
def surface_area(lower_left,middle_right,thickness,extrusion_length,with_inner_leg=False,test=False,XZ_face_only=False,extrusion_area_only=False):
    ...
    if with_inner_leg == True:
        # XZ plane face area
        total_face_area += thickness*height
        print(total_face_area)
        total_leg_surface_area = 2*height*thickness + 2*extrusion_length*height + 2*extrusion_length*thickness
...
def volume(lower_left,middle_right,thickness,extrusion_length,with_inner_leg=False,test=False):
    """
    ...
    face_area = surface_area(lower_left,middle_right,thickness,extrusion_length,test=False,XZ_face_only=True,with_inner_leg=with_inner_leg)
    print(face_area)
