...
class test_PoloidalFieldCoilCaseFC(unittest.TestCase):
    ...
    def test_PoloidalFieldCoilCaseFC_absolute_volume(self):
        ...
        assert test_shape.volume == pytest.approx((math.pi*2*1000) * ((50*5*2) + (70*5*2)))
    ...
    def test_PoloidalFieldCoilCaseFC_absolute_areas(self):
        ...
        assert test_shape.areas.count(pytest.approx(70*math.pi*2*1000)) == 2
        ...
        assert test_shape.areas.count(pytest.approx(60*math.pi*2*1035)) == 1
        assert test_shape.areas.count(pytest.approx(60*math.pi*2*965)) == 1
